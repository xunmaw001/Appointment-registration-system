<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="调班时间" prop="diaobanshijian">
          <el-select v-model="ruleForm.diaobanshijian" placeholder="请选择调班时间">
            <el-option
                v-for="(item,index) in diaobanshijianOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="调班时间" prop="diaobanshijian">
	      <el-input v-model="ruleForm.diaobanshijian"
                placeholder="调班时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="调班原因" prop="diaobanyuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="调班原因"
                  v-model="ruleForm.diaobanyuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.diaobanyuanyin" label="调班原因" prop="diaobanyuanyin">
                    <span>{{ruleForm.diaobanyuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	zhibanshijian : false,
	lianxidianhua : false,
	diaobanyuanyin : false,
	diaobanshijian : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.lianxidianhua = true;
          ...
          if(o=='diaobanyuanyin'){
            this.ruleForm.diaobanyuanyin = obj[o];
	    this.ro.diaobanyuanyin = true;
            continue;
          }
          if(o=='diaobanshijian'){
            this.ruleForm.diaobanshijian = obj[o];
	    this.ro.diaobanshijian = true;
            continue;
          }
		...
		if(json.touxiang!=''&&json.touxiang){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.diaobanshijianOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `diaobanshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.diaobanshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
